...
const AdminSubscriptions = () => {
  ...
  const subs = Array.isArray(data?.data) ? data.data : (Array.isArray(data) ? data : [])
  const total = typeof data?.total === 'number' ? data.total : subs.length

  const planCount = (plan: string) =>
    subs.filter((s: any) => s.plan === plan).length

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white">Subscriptions</h1>
        <p className="text-slate-400 text-sm mt-1">{total} total subscription records</p>
      </div>
