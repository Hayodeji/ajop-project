...
export const adminGetUsers = (page = 1, search?: string) =>
  api.get('/admin/users', { params: { page, limit: 20, search } }).then(r => r.data)

export const adminGetUser = (userId: string) =>
  api.get(`/admin/users/${userId}`).then(r => r.data.data ?? r.data)

export const adminUpdateUser = (userId: string, body: { plan?: string; role?: string; suspended?: boolean }) =>
  api.patch(`/admin/users/${userId}`, body).then(r => r.data.data ?? r.data)

export const adminGetGroups = (page = 1, search?: string) =>
  api.get('/admin/groups', { params: { page, limit: 20, search } }).then(r => r.data)
...
export const adminGetSubscriptions = (page = 1) =>
  api.get('/admin/subscriptions', { params: { page, limit: 20 } }).then(r => r.data)
...
export const adminDeleteUser = (userId: string) =>
  api.delete(`/admin/users/${userId}`).then(r => r.data.data ?? r.data)

export const adminGetEngagement = () =>
  api.get('/admin/engagement').then(r => r.data.data ?? r.data)
